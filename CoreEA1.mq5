...
// --- Scale-Out Tracking Variables (added to match Pine Script) ---
// Scale-out tracking: remember which tickets have had Scale-Out1 applied
static ulong  g_scaleOut1TriggeredTickets[];    

// --- Trade Entry Throttling Variables ---
...
double CalculateLotSize(double stop_loss_distance_points) 
  {
   ...
   // --- FIX: scale XAUUSD lot_size *before* normalisation & rounding ---
...
int OnInit()
  {
   ...
   // Initialize scale-out tracking array for existing positions
   // This only populates the tracking array on initialization without sending any trades
   ...
   // Restore scale-out tracking for any existing EA positions on re-init
   RestoreScaleOutTrackingForExistingPositions();
...
void OnTick()
  {
               ...
               if(g_point_value > 0 && sl_distance_points_for_lot > g_point_value) 
                 {
                  ...
                  if(trade_lot_size >= SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
                    {
                     // Only attempt a new trade when the signal changes (or every 60s at most)
                     if(signal != prevSignal && TimeCurrent() - lastAttemptTime > 60)
                     {
                        ...
                        if(OpenTrade(signal, trade_lot_size, sl_price, tp_price, trade_comment))
   ...
   // --- replace one-off scale-out with loop for all tickets ---
   if(InpScaleOut1Enabled)  // assuming this flag controls your first scale-out step
   {
      CTrade trade;
      // walk every position in the account
      for(int pos=PositionsTotal()-1; pos>=0; pos--)
      {
         ...
         if(IsTicketScaled(ticket)) continue;

         double entry   = PositionGetDouble(POSITION_PRICE_OPEN);
         double tp      = PositionGetDouble(POSITION_TP);
         double vol     = PositionGetDouble(POSITION_VOLUME);
         double current = (PositionGetInteger(POSITION_TYPE)==ORDER_TYPE_BUY)
                          ? SymbolInfoDouble(_Symbol,SYMBOL_BID)
                          : SymbolInfoDouble(_Symbol,SYMBOL_ASK);

         // compute distances
         double fullDist = MathAbs(tp - entry);
         double currDist = MathAbs(current - entry);

         // if we've hit the configured % of total TP distance
         if(fullDist>0 && currDist >= fullDist*(InpScaleOut1Pct/100.0))
         {
            double   toClose = vol*(InpScaleOut1Size/100.0);
            // protect against tiny closes
            if(toClose > SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN))
            {
               if(trade.PositionClosePartial(ticket, toClose))
               {
                  PrintFormat("[ScaleOut1] closed %.2f lots on ticket %d", toClose, ticket);
                  // mark this ticket done
                  int n = ArraySize(g_scaleOut1TriggeredTickets);
                  ArrayResize(g_scaleOut1TriggeredTickets, n+1);
                  g_scaleOut1TriggeredTickets[n] = ticket;
               }
            }
         }
      }
...
void ResetScaleOutFlags()
{
    ...
    ArrayFree(g_scaleOut1TriggeredTickets);
    Print("[ScaleOut] all scale-out tickets reset");
}
...
void RestoreScaleOutTrackingForExistingPositions()
{
   ...
   // Mark each open EA position as already scaled
   ...
   PrintFormat("[ScaleOut] Restored scale-out tracking for %d existing positions", ArraySize(g_scaleOut1TriggeredTickets));
}
...
//+------------------------------------------------------------------+
//| Helper to test if we've already scaled out a given ticket       |
//+------------------------------------------------------------------+
bool IsTicketScaled(ulong ticket)
{
   for(int i=0; i<ArraySize(g_scaleOut1TriggeredTickets); i++)
      if(g_scaleOut1TriggeredTickets[i] == ticket)
         return true;
   return false;
}